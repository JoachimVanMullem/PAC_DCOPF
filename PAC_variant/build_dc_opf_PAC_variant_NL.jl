function build_dc_opf!(m::Model)
    ...
    ### THIS FUNCTION BUILDS THE MODEL OF A PAC-variant AGENT
    ##### NONLINEAR CONSTRAINT MODEL --> nonlinear constraint added in nodal power balance
    ...
    #maak variable bus_ang copies fsow? zoveel dual variables komen er ook later van 
    ...
    #nodal power balance
    m.ext[:constraints][:nodal_balance] = @NLconstraint(m, [t=T],
        sum(gen_p[gen_id,t] for gen_id in G) 
        - sum(load_p[load_id][t] for load_id in L)
        - sum(-br_b[br] * sin(bus_ang[i, t] - bus_ang_copy[j, t]) for (br,i,j) in arcs_fr) 
        == 0) 
