function init_model_bus!(m::Model, m_central::Dict,bus_id::String,time_steps::Int)
    ...
    ### THIS FUNCTION INITIALIZES THE MODEL OF AN ADMM AGENT
    ...
    #List of connected node (Neighbours in the system)
    ...
    #Branch properties
    s_max_a = m.ext[:sets][:branch][:s_max_a] = Dict(p for p in br_s_max_a_central if p.first in BR)
    ...
    m.ext[:sets][:gen][:name]= Dict(p for p in gen_name_central if p.first in G) 
    #normaal niet meer nodig
    m.ext[:sets][:gen][:bus] = Dict(p  for p in gen_bus_central if p.first in G)

    #LOADS 
    #List of loads connected to this bus
    ...
    return G, cost_linear,arcs_fr, arcs_to, load_p , M_connected ,BR, b ,s_max_a
